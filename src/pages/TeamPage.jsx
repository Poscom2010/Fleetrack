...
const TeamPage = () => {
  ...
  const getDriverUserStatus = (driverProfile) => {
    const hasUser = users.some(u => {
      const matchByUserId = driverProfile.userId && u.uid === driverProfile.userId;
      const matchByEmail = driverProfile.email && u.email && 
                          u.email.toLowerCase() === driverProfile.email.toLowerCase();
      return matchByUserId || matchByEmail;
    });
    
    const hasPendingInvite = invitations.some(inv => 
      (driverProfile.email && inv.email && inv.email.toLowerCase() === driverProfile.email.toLowerCase()) || 
      inv.driverProfileId === driverProfile.id
    );
    
    if (hasUser) {
      return { 
        status: 'registered', 
        user: users.find(u => u.email?.toLowerCase() === driverProfile.email?.toLowerCase()) 
      };
    } else if (hasPendingInvite) {
      return { 
        status: 'pending_invite', 
        invitation: invitations.find(inv => inv.email?.toLowerCase() === driverProfile.email?.toLowerCase()) 
      };
    } else {
      return { status: 'not_invited', driver: driverProfile };
    }
  };
    ...
    try {
      ...
      const invitationsData = updatedInvitations
        .filter(inv => {
          ...
          if (!inv.email || !inv.fullName) return false;
          return true;
        })
        .map(inv => {
          try {
  ...
  const loadDriverStats = async () => {
    try {
      ...
      // Load stats from dailyEntries (traditional taxi/courier trips)
      let dailyEntriesQuery;
      if (isSystemAdmin) {
        dailyEntriesQuery = query(collection(db, 'dailyEntries'));
      } else {
        dailyEntriesQuery = query(
          collection(db, 'dailyEntries'),
          where('companyId', '==', company.id)
        );
      }

      const dailyEntriesSnapshot = await getDocs(dailyEntriesQuery);

      dailyEntriesSnapshot.forEach((docSnap) => {
        ...
        stats[driverId].totalKm += distance;
        if (vehicleId) stats[driverId].vehiclesUsed.add(vehicleId);
      });

      // Load stats from loadEvents (commodity trips)
      let loadEventsQuery;
      if (isSystemAdmin) {
        loadEventsQuery = query(collection(db, 'loadEvents'));
      } else {
        loadEventsQuery = query(
      ...
      loadEventsSnapshot.forEach((docSnap) => {
        const data = docSnap.data();
      ...
      // Now get the actual total distance from vehicle odometer readings
      // For each driver, sum up the current odometer of all vehicles they've used
      for (const driverId in stats) {
        const vehiclesUsed = Array.from(stats[driverId].vehiclesUsed);
        let totalDistance = 0;
        ...
        stats[driverId].totalKm = totalDistance;
      }
  ...
  // Also include driver profiles who aren't invited yet (regardless of trip data)
  const teamMembers = React.useMemo(() => {
    const members = [...users];
    
    // Add uninvited driver profiles
    driverProfiles.forEach(profile => {
      // Check if this profile is already a registered user
      const isAlreadyUser = users.some(u => 
        (profile.userId && u.uid === profile.userId) ||
        (profile.email && u.email && u.email.toLowerCase() === profile.email.toLowerCase())
      );
      
      // Only add if not already a registered user
      if (!isAlreadyUser) {
        members.push({
          id: profile.id,
  ...
  // Handler for inviting an existing driver
  const handleInviteExistingDriver = (driverProfile) => {
    setInviteForm({
      email: driverProfile.email || '',
      fullName: driverProfile.name || driverProfile.fullName || '',
      phoneNumber: driverProfile.phoneNumber || '',
      location: driverProfile.location || '',
      role: 'company_user',
      driverProfileId: driverProfile.id // Link to driver profile
    });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-baltic-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-4">
      <div className="max-w-7xl mx-auto space-y-4">
        {/* Page Header */}
        <div className="mb-4">
          <h1 className="text-2xl font-bold text-baltic-900 dark:text-white">Team Management / Invitations</h1>
          <p className="text-sm text-gray-600 dark:text-gray-400">Manage team members, invite drivers, and monitor activity</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 shadow-lg">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-baltic-900 dark:text-white">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition ${
                  showPendingInvites 
                    ? 'bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-900 dark:text-white border-2 border-gray-300 dark:border-gray-600' 
                    : 'bg-amber-500 hover:bg-amber-600 text-white shadow-lg border-2 border-amber-600'
                }`}
              >
              ...
              {!showPendingInvites && (
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-baltic-500 hover:bg-baltic-600 dark:bg-baltic-600 dark:hover:bg-baltic-700 text-white rounded-lg font-medium transition shadow-md hover:shadow-lg"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {loading ? (
            ...
            <>
              ...
              {invitations.length === 0 ? (
                <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
                  <svg className="mx-auto h-12 w-12 text-gray-500 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                  <h3 className="mt-2 text-sm font-medium text-baltic-900 dark:text-gray-100">No pending invitations</h3>
                  <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">All team members have accepted their invitations!</p>
                </div>
              ) : (
                <>
                  ...
                  <div className="lg:hidden space-y-3">
                    {invitations.map((invitation) => (
                      <div key={invitation.id} className="bg-white dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600 p-4 shadow-sm">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            <h3 className="text-baltic-900 dark:text-white font-semibold">{invitation.fullName}</h3>
                            <p className="text-xs text-gray-600 dark:text-gray-400">{invitation.email}</p>
                          </div>
                          <span className="px-2 py-1 rounded text-xs font-medium bg-warning/20 text-warning dark:text-warning border border-warning/30">
                            📧 Pending
                          </span>
                        </div>
                        ...
                        <div className="space-y-2 text-sm mb-3">
                          <div>
                            ...
                            <p className="text-baltic-900 dark:text-white font-medium">{invitation.role === 'company_admin' ? 'Admin' : 'Driver'}</p>
                          </div>
                          <div>
                            <span className="text-gray-600 dark:text-gray-400">Phone:</span>
                            <p className="text-baltic-900 dark:text-white font-medium">{invitation.phoneNumber || 'Not provided'}</p>
                          </div>
                          <div>
                            <span className="text-gray-600 dark:text-gray-400">Invited:</span>
                            <p className="text-baltic-900 dark:text-white font-medium">{invitation.createdAt ? new Date(invitation.createdAt.seconds * 1000).toLocaleDateString() : 'Unknown'}</p>
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          className="w-full px-3 py-2 bg-danger hover:bg-danger/90 dark:bg-danger dark:hover:bg-danger/80 text-white rounded-lg text-sm font-medium transition flex items-center justify-center gap-2">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="hidden lg:block overflow-x-auto">
                    <table className="w-full">
                      <thead className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                        <tr>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-400 uppercase">Name</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-400 uppercase">Email</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-400 uppercase">Phone</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-400 uppercase">Role</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-400 uppercase">Invited</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-400 uppercase">Status</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-400 uppercase">Actions</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-700">
                        {invitations.map((invitation) => (
                          <tr className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition border-b border-gray-200 dark:border-gray-700">
                            <td className="px-4 py-3 text-sm text-baltic-900 dark:text-white">{invitation.fullName}</td>
                            <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{invitation.email}</td>
                            <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{invitation.phoneNumber || 'Not provided'}</td>
                            <td className="px-4 py-3">
                              <span className={`px-2 py-1 rounded text-xs font-medium ${
                                invitation.role === 'company_admin' 
                                  ? 'bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-300 border border-purple-300 dark:border-purple-500/30'
                            ...
                            <td className="px-4 py-3">
                              <button
                                onClick={() => handleDeleteInvitation(invitation.id, invitation.email, invitation.fullName)}
                                className="px-3 py-1.5 bg-danger hover:bg-danger/90 dark:bg-danger dark:hover:bg-danger/80 text-white rounded text-xs font-medium transition flex items-center gap-1"
            ...
            <>
              ...
              <div className="lg:hidden space-y-3">
                {teamMembers.length === 0 ? (
                  <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
                    ...
                    <p className="mt-4 text-gray-600 dark:text-gray-400">No drivers yet</p>
                    <p className="text-xs text-gray-500 dark:text-gray-500 mt-1">Invite drivers from Company Settings</p>
                  </div>
                ) : (
                  teamMembers.map((driver) => {
                    ...
                    // Hide last login for admins viewing managers (and vice versa)
                    const shouldHideLastLogin = !isSystemAdmin && 
                    ...
                    return (
                      <div key={driver.id} className="bg-white dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600 p-3 shadow-sm">
                        {/* Driver Info */}
                        <div className="flex items-center gap-3 mb-3 pb-3 border-b border-gray-200 dark:border-gray-600">
                          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-baltic-500 shadow-lg">
                            <svg className="h-6 w-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                            </svg>
                          </div>
                          <div className="flex-1">
                            <p className="font-medium text-baltic-900 dark:text-white text-sm">
                              {driver.fullName || driver.displayName || driver.email}
                            </p>
                            <p className="text-xs text-gray-600 dark:text-gray-400">{driver.email}</p>
                          </div>
                        </div>
                        ...
                        <div className="mb-3">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-gray-600 dark:text-gray-400 text-xs">Vehicle{vehicleCount > 1 ? 's' : ''}:</span>
                            {vehicleCount > 1 && (
                          ...
                          {stats.vehiclesUsed && stats.vehiclesUsed.length > 0 ? (
                            vehicleCount === 1 ? (
                              ...
                              (() => {
                                ...
                                return vehicle ? (
                                  <div className="flex items-center gap-2 text-xs bg-green-50 dark:bg-slate-800 px-3 py-2 rounded border border-green-200 dark:border-slate-600">
                                    <svg className="w-4 h-4 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                                    </svg>
                                    <span className="text-baltic-900 dark:text-white font-medium">{vehicle.name} ({vehicle.registrationNumber})</span>
                                  </div>
                                ) : <p className="text-xs text-gray-600 dark:text-gray-400">Vehicle not found</p>;
                              })()
                            ) : (
                              ...
                              <div className="space-y-2">
                                {stats.vehiclesUsed.map(vehicleId => {
                                  ...
                                  return vehicle ? (
                                    <div key={vehicleId} className="flex items-center gap-2 text-xs bg-blue-50 dark:bg-slate-800 px-2 py-1.5 rounded border border-blue-200 dark:border-slate-600">
                                      <svg className="w-3 h-3 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                      </svg>
                                      <span className="text-baltic-900 dark:text-white">{vehicle.name} ({vehicle.registrationNumber})</span>
                                    </div>
                                  ) : null;
                                })}
                                <p className="text-xs text-gray-600 dark:text-gray-500 italic">Used multiple vehicles</p>
                              </div>
                            )
                          ) : (
                            <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">No trips captured yet</p>
                          )}
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-3 text-xs">
                          <div>
                            ...
                            <p className="text-baltic-900 dark:text-white font-semibold">{stats.totalKm.toFixed(2)} km</p>
                          </div>
                          <div>
                            <span className="text-gray-600 dark:text-gray-400">Last Login:</span>
                            {shouldHideLastLogin ? (
                              <p className="text-xs text-gray-600 dark:text-gray-500 mt-1">—</p>
                            ) : (
                              <div className="flex items-center gap-1 mt-1">
                                ...
                                <span className={`text-xs ${
                                  loginStatus === 'active' ? 'text-baltic-900 dark:text-white' :
                        ...
                        {/* Role */}
                        <div className="mb-3">
                          <span className="text-gray-600 dark:text-gray-400 text-xs">Role:</span>
                          {driver.isDriverProfile ? (
                            <div className="mt-1">
                              <span className="inline-block px-3 py-1.5 rounded text-xs font-medium bg-amber-100 dark:bg-yellow-500/20 text-amber-700 dark:text-yellow-300 border border-amber-300 dark:border-yellow-500/30">
                                ⏳ Pending Invitation
                              </span>
                              <p className="text-xs text-gray-600 dark:text-gray-500 mt-1">Driver has trip data but hasn't been invited yet</p>
                            </div>
                          ) : driver.id === user?.uid ? (
              ...
              {/* Desktop Table View */}
              <div className="hidden lg:block overflow-x-auto rounded-lg border border-gray-200 dark:border-slate-700">
                <table className="w-full">
                  <thead className="bg-gray-50 dark:bg-slate-900/50 border-b border-gray-200 dark:border-slate-700">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-700 dark:text-slate-400">
                        Driver
                      </th>
                      <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-700 dark:text-slate-400">
                        Vehicles Used
                      </th>
                      <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wide text-gray-700 dark:text-slate-400">
                        # Vehicles
                      </th>
                      <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide text-gray-700 dark:text-slate-400">
                        Total km travelled
                      </th>
                      <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-700 dark:text-slate-400">
                        Last system login
                  ...
                  {teamMembers.length === 0 ? (
                    <tr>
                      <td colSpan="7" className="px-4 py-12 text-center">
                        ...
                        <p className="mt-4 text-gray-600 dark:text-slate-400">No drivers yet</p>
                        <p className="text-xs text-gray-500 dark:text-slate-500 mt-1">Invite drivers from Company Settings</p>
                      </td>
                    </tr>
                  ) : (
                    teamMembers.map((driver) => {
                      ...
                      return (
                        <tr key={driver.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/60 transition">
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-baltic-600 dark:bg-blue-600 shadow-lg">
                                <svg className="h-6 w-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                </svg>
                              </div>
                              <div className="flex flex-col">
                                <span className="font-medium text-baltic-900 dark:text-slate-100">
                                  {driver.fullName || driver.displayName || driver.email}
                                </span>
                                <span className="text-xs text-gray-600 dark:text-slate-400">{driver.email}</span>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            {stats.vehiclesUsed && stats.vehiclesUsed.length > 0 ? (
                              vehicleCount === 1 ? (
                                ...
                                (() => {
                                  ...
                                  return vehicle ? (
                                    <div className="flex items-center gap-2 text-sm">
                                      ...
                                      <span className="text-gray-900 dark:text-slate-200 font-medium">{vehicle.name} ({vehicle.registrationNumber})</span>
                                    </div>
                                  ) : <span className="text-xs text-gray-500 dark:text-slate-400">Vehicle not found</span>;
                                })()
                              ) : (
                                ...
                                <div className="space-y-1">
                                  {stats.vehiclesUsed.map(vehicleId => {
                                    const vehicle = vehicles.find(v => v.id === vehicleId);
                                    return vehicle ? (
                                      <div key={vehicleId} className="flex items-center gap-1.5 text-xs bg-gray-100 dark:bg-slate-800 px-2 py-1 rounded border border-gray-300 dark:border-slate-600">
                                        <svg className="w-3 h-3 text-blue-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                        </svg>
                                        <span className="text-gray-900 dark:text-slate-200">{vehicle.name} ({vehicle.registrationNumber})</span>
                                      </div>
                                    ) : null;
                                  })}
                                </div>
                              ...
                              <span className="text-xs text-gray-500 dark:text-slate-400">No trips yet</span>
                            )}
                          </td>
                          ...
                          <td className="px-4 py-3">
                            {shouldHideLastLogin ? (
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className={`text-xs ${
                                  loginStatus === 'active' ? 'text-gray-900 dark:text-slate-200' :
                                  loginStatus === 'warning' ? 'text-orange-600 dark:text-orange-400' :
                                  loginStatus === 'inactive' ? 'text-red-600 dark:text-red-400' :
                                  loginStatus === 'pending' ? 'text-yellow-600 dark:text-yellow-400' :
                          ...
                          <td className="px-4 py-3">
                            {driver.isDriverProfile ? (
                              <span className="px-3 py-1.5 rounded text-xs font-medium bg-amber-100 dark:bg-yellow-500/20 text-amber-700 dark:text-yellow-300 border border-amber-300 dark:border-yellow-500/30">
                                ⏳ Pending Invitation
                              </span>
                            ) : driver.id === user?.uid ? (
        ...
        {teamMembers.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 shadow-md">
            <h3 className="text-xs font-semibold text-baltic-900 dark:text-gray-100 uppercase tracking-wide mb-3">Activity Status</h3>
            <div className="flex flex-wrap gap-4 text-xs">
              <div className="flex items-center gap-2">
                <div className="h-2 w-2 rounded-full bg-green-500" />
                <span className="text-gray-700 dark:text-gray-300 font-medium">Active (logged in within 14 days)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="h-2 w-2 rounded-full bg-orange-500" />
                <span className="text-gray-700 dark:text-gray-300 font-medium">Warning (14-30 days since login)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="h-2 w-2 rounded-full bg-red-500" />
                <span className="text-gray-700 dark:text-gray-300 font-medium">Inactive (30+ days since login)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="h-2 w-2 rounded-full bg-gray-500" />
                <span className="text-gray-700 dark:text-gray-300 font-medium">Never logged in</span>
              </div>
            </div>
          </div>
        )}
        ...
        {/* Invite Driver Modal */}
        {showInviteModal && (
          <div className="fixed inset-0 bg-black/60 dark:bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-md p-4 border border-gray-200 dark:border-slate-700 my-4 max-h-[95vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-bold text-baltic-900 dark:text-white">Invite Team Member</h3>
                <button
                  onClick={() => setShowInviteModal(false)}
                  className="text-gray-500 dark:text-slate-400 hover:text-baltic-900 dark:hover:text-white transition"
                >
              ...
              <form onSubmit={handleInviteDriver} className="space-y-3">
                ...
                {driverProfiles.length > 0 && (
                  <div className="bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/30 rounded-lg p-3 mb-3">
                    ...
                    <select
                      ...
                      className="w-full bg-white dark:bg-slate-700 text-gray-900 dark:text-white px-4 py-2 rounded-lg border border-gray-300 dark:border-slate-600 focus:border-baltic-500 focus:ring-2 focus:ring-baltic-500 focus:outline-none"
                    >
                    ...
                    <p className="text-xs text-blue-600 dark:text-blue-200 mt-2">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white dark:bg-slate-700 text-gray-900 dark:text-white px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 focus:border-baltic-500 focus:ring-2 focus:ring-baltic-500 focus:outline-none text-sm"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                  />
                  <p className="text-xs text-blue-600 dark:text-blue-200 mt-1 flex items-center gap-1">
                ...
                <div className="flex gap-2 pt-3">
                  <button
                    type="button"
                    onClick={() => setShowInviteModal(false)}
                    className="flex-1 px-3 py-2 bg-gray-200 dark:bg-slate-700 hover:bg-gray-300 dark:hover:bg-slate-600 text-gray-900 dark:text-white rounded-lg font-medium transition text-sm"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-1 px-3 py-2 bg-baltic-600 hover:bg-baltic-700 text-white rounded-lg font-medium transition text-sm"
                  >
                    Send Invitation
                  </button>
                </div>
