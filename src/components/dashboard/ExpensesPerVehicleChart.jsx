...
/**
 * ExpensesPerVehicleChart component to display expenses per vehicle over time
 * @param {Object} props
 * @param {Array} props.expenses - Array of expenses with vehicleId, date, amount
 * @param {Array} props.vehicles - Array of vehicles with id and name
 * @param {string} props.period - Time period: 'day', 'week', 'month', 'year'
 */
const ExpensesPerVehicleChart = ({ expenses = [], vehicles = [], period = "day" }) => {
  const chartData = useMemo(() => {
    if (!expenses.length || !vehicles.length) return [];

    // Determine date range based on expenses
    ...
    // Group expenses by time period and vehicle
    const groupedData = timePeriods.map((periodStart) => {
      ...
      // Calculate expenses for each vehicle in this period
      vehicles.forEach((vehicle) => {
        const vehicleExpenses = expenses.filter((expense) => {
        ...
        const totalExpenses = vehicleExpenses.reduce((sum, expense) => sum + (expense.amount || 0), 0);
        dataPoint[vehicle.name] = totalExpenses;
      });
    ...
    return groupedData;
  }, [expenses, vehicles, period]);

  // Generate colors for each vehicle (same as revenue chart for consistency)
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-surface-200/60 p-6 shadow-soft">
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-white">Expenses per Vehicle</h3>
        <p className="mt-1 text-sm text-slate-400">Total expenses for each vehicle over time</p>
      </div>
...
export default ExpensesPerVehicleChart;
